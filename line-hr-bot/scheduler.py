...
# วันหยุดนักขัตฤกษ์และวันหยุดบริษัท ประจำปี 2569 (ค.ศ. 2026)
_HOLIDAYS_2026 = {
    ...
    "2026-06-03",  # วันเฉลิมพระชนมพรรษา สมเด็จพระนางเจ้าสุทิดา พระบรมราชินี
...
def _is_weekday() -> bool:
    """คืน True ถ้าวันนี้เป็นวันทำงาน (จันทร์–ศุกร์ และไม่ใช่วันหยุดนักขัตฤกษ์)"""
    if _now_thai().weekday() >= 5:   # 5=เสาร์, 6=อาทิตย์
        return False
    return _today_thai() not in _HOLIDAYS_2026
...
def _run_workplan_check():
    if not _is_weekday():
        logger.info("[SCHEDULER] วันหยุดสุดสัปดาห์ — ข้ามการตรวจแผนงาน")
        return
    today = _today_thai()
    logger.info(f"[SCHEDULER] ตรวจแผนงาน {today} {WORKPLAN_DEADLINE} น.")
    # ดึงข้อมูลเข้างานล่าสุดก่อนตรวจแผนงาน
    try:
        from processors.humansoft_api import fetch_attendance
        asyncio.run(fetch_attendance(today))
        logger.info("[SCHEDULER] fetch_attendance (workplan) สำเร็จ")
    except Exception as e:
        logger.error(f"[SCHEDULER fetch_attendance workplan] {e}")
    try:
...
def _run_daily_report():
    if not _is_weekday():
        logger.info("[SCHEDULER] วันหยุดสุดสัปดาห์ — ข้ามการส่ง report เข้า LINE")
        return
    from generators.report_generator import generate_and_send_report
    today = _today_thai()
    logger.info(f"[SCHEDULER] เริ่มสร้าง report วันที่ {today}")
    # ดึงข้อมูลเข้างานล่าสุดจาก HumanSoft ก่อนสร้าง report
    try:
        from processors.humansoft_api import fetch_attendance
        asyncio.run(fetch_attendance(today))
        logger.info("[SCHEDULER] fetch_attendance (report) สำเร็จ")
    except Exception as e:
        logger.error(f"[SCHEDULER fetch_attendance report] {e}")
    # ส่ง report ไม่ว่า fetch จะสำเร็จหรือไม่
    try:
