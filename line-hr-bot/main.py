...
# ─── Debug Name Match (เปรียบเทียบชื่อ API vs staff_mapping) ────────────────────

@app.get("/debug-name-match")
async def debug_name_match(target_date: str = Query(default=None, alias="date")):
    """เปรียบเทียบชื่อจาก HumanSoft API กับ staff_mapping.csv
       คืน: matched, unmatched_api (ชื่อ API ที่ไม่มีใน CSV), unmatched_csv (ชื่อ CSV ที่ไม่มีใน API)
    """
    import httpx
    td = target_date or date.today().isoformat()
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(
            ...
            params={"path_action": "list_employee_inout_daily", "work_date": td,
                    "employee_code": hs_emp, "language_code": "TH"},
        )
    data = resp.json()
    items = data.get("payload", data.get("data", data.get("result", []))) if isinstance(data, dict) else data
    api_names = {
        f"{(e.get('employee_name') or '').strip()} {(e.get('employee_last_name') or '').strip()}".strip()
        for e in items
    }

    # ดึงชื่อจาก staff_mapping
    from processors.staff_mapper import get_all_staff_names, reload
    reload()
    csv_names = {name for name, _, _ in get_all_staff_names()}

    matched         = sorted(api_names & csv_names)
    unmatched_api   = sorted(api_names - csv_names)   # มีใน API แต่ไม่มีใน CSV
    unmatched_csv   = sorted(csv_names - api_names)   # มีใน CSV แต่ไม่มีใน API
