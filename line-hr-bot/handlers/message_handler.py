...
from processors.staff_mapper import get_employee_name, reload as reload_staff
...
async def handle_text(group_id: str, user_id: str, text: str):
    ...
    line_name = await _get_user_display_name(user_id, group_id)
    sender_name = get_employee_name(user_id, fallback=line_name)
    ...
    if stripped == "/myid":
        await _push_text_to_group(group_id, f"🪪 {line_name}\nUser ID: {user_id}")
        return

    if stripped == "/reload-staff":
        count = reload_staff()
        await _push_text_to_group(group_id, f"✅ โหลด Staff Mapping สำเร็จ {count} คน")
        return
