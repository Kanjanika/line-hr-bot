...
def _extract_checkin(time_list: list) -> str:
    ...
    HumanSoft API ใช้ field: attendance_datetime (format: YYYY-MM-DD HH:MM:SS)
    """
    all_times = [t.get("attendance_datetime", "") for t in time_list if t.get("attendance_datetime")]
    if all_times:
        return sorted(all_times)[0][11:16]  # "HH:MM"
    return ""
