...
def _parse_excel(file_path: str, default_date: str) -> int:
    ...
    for row in all_rows[header_row_idx + 1:]:
        ...
        if any(kw in excel_status for kw in leave_keywords):
            status = "leave"
            if any(kw in excel_status for kw in {"ป่วย", "sick", "ลาป่วย"}):
                leave_type_val = "ลาป่วย"
            elif any(kw in excel_status for kw in {"พักร้อน", "annual", "vacation", "ลาพักร้อน"}):
                leave_type_val = "ลาพักร้อน"
            elif any(kw in excel_status for kw in {"กิจ", "personal", "ลากิจ"}):
                leave_type_val = "ลากิจ"
            else:
                leave_type_val = "อื่นๆ"
            ...
            # เช็คมาสาย: เข้างานหลัง 08:30 น.
            check_in_norm = _normalize_time(in_raw)
            if check_in_norm and check_in_norm > "08:30":
                status = "late"
            else:
                status = "present"
        else:
