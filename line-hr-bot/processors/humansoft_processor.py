...
def _parse_excel(file_path: str, default_date: str) -> int:
    ...
    name_idx = date_idx = id_idx = in_idx = out_idx = remark_idx = None
    ...
    logger.info(
        ...
        f"id={id_idx} in={in_idx} out={out_idx} remark={remark_idx}"
    )
    ...
    for row in all_rows[header_row_idx + 1:]:
        ...
        # status: ถ้า check-in มี Google Maps → ประชุม/WFH
        status = "present" if in_raw else "absent"
        if in_raw and maps_url:
            status = "meeting"
